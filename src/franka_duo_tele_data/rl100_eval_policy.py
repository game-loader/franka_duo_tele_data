...
def _validate_inputs(
    ...
    if missing_wrist:
        raise ValueError(
            f"manifest.inputs.image_keys must consume both wrist RGB streams; missing {sorted(missing_wrist)}"
        )
    ...
    required_keys = (
        (point_key,) + ((str(raw["state_key"]),) if raw.get("state_key") else ()) + image_model_keys
    )
...
def _load_lerobot(bundle_dir: Path, manifest: dict[str, Any], device: str) -> PolicyBundle:
    try:
        ...
        raise RuntimeError(
            "The LeRobot policy backend requires LeRobot to be installed; "
            "use an rl100_native bundle for the standalone runtime"
        ) from exc
...
class _NativePredictor:
    ...
    def _history_batch(self, observation: Mapping[str, np.ndarray]) -> dict[str, Any]:
        ...
        batch: dict[str, Any] = {}
        for key, value in observation.items():
            tensor = _prepare_native_input({key: value}, self.device, image_keys=self.image_keys)[key]
...
def load_policy_bundle(
    ...
    *,
    ...
    contract_path = (
        Path(manifest_path).expanduser().resolve() if manifest_path else bundle_dir / MANIFEST_NAME
    )
