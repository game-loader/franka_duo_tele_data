...
log_file="${root_dir}/localization.log"
if [[ -f "${pid_file}" ]] && kill -0 "$(cat "${pid_file}")" 2>/dev/null; then
  echo "localization already running (PID $(cat "${pid_file}"))"
  exit 0
fi
...
setsid ros2 launch tmr_local_navigation localization.launch.py \
  >"${log_file}" 2>&1 < /dev/null &
pid=$!
echo "${pid}" >"${pid_file}"
echo "localization started (PID ${pid}); log: ${log_file}"
